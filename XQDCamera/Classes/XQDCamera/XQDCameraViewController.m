...
-(void)viewDidLayoutSubviews{
    ...
    if (_img_placeholder.width>270&&_placeHolderType!=XQDCameraViewPlaceholderTypeWithCard) {
    ...
    if(IS_Iphone6p_Or_Later&&_placeHolderType==XQDCameraViewPlaceholderTypeWithCard){
...
- (void)viewDidLoad {
    ...
    if (_placeHolderType==XQDCameraViewPlaceholderTypeFront) {
//        _img_placeholder.image=[UIImage imageNamed:@"1080-1920-正面"];
        _img_placeholder.image = getImageWithImageName(@"1080-1920-正面");
        _tip.hidden=NO;
    }
    else if(_placeHolderType==XQDCameraViewPlaceholderTypeBack){
//        _img_placeholder.image=[UIImage imageNamed:@"1080-1920-背面"];
        _img_placeholder.image = getImageWithImageName(@"1080-1920-背面");
        _tip.hidden=NO;
    }
    else{
//        _img_placeholder.image=[UIImage imageNamed:@"1080-1920-手持"];
        _img_placeholder.image = getImageWithImageName(@"1080-1920-手持");
        _tip.hidden=YES;
    ...
    self.camera = [[LLSimpleCamera alloc] initWithQuality:AVCaptureSessionPresetHigh
                                                 position:_placeHolderType==XQDCameraViewPlaceholderTypeWithCard? CameraPositionFront:CameraPositionBack
                                             videoEnabled:NO];
...
- (IBAction)snapButtonPressed:(UIButton *)button {
    ...
    [self.camera capture:^(LLSimpleCamera *camera, UIImage *image, NSDictionary *metadata, NSError *error) {
        if(!error) {
            ...
            [camera stop];
            _image=image;
            XQDCameraHolderViewController* imageViewController= [[XQDCameraHolderViewController alloc] init];
            if (self.delegate) {
                imageViewController.delegate=self.delegate;
            } else if(self.finishBlock) {
                imageViewController.finishBlock = self.finishBlock;
            }
            imageViewController.placeHolderType=(XQDCameraHolderViewPlaceholderType)self.placeHolderType;
